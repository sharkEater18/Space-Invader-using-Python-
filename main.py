...
# Scoring
score = 0
score_panel = turtle.Turtle(visible=False)
score_panel.speed(0)
...
# Invader(s)
difficulty_factor = 10 # 1 for testing purpose###################################
number_of_invaders = 1 * difficulty_factor
invaders = []
...
# Game Loop
# play_sound('meagalovania.wav', 313) # For Linux /Mac use this 
...
game_over = False
one_time = 0 # Parameter to show the turtle objects
hit = 0
while 1:
    wn.update()


    if hit == number_of_invaders and number_of_invaders <= 3 * difficulty_factor:
        wn.bgpic('ge.gif')
        wn.update()
        time.sleep(2)
        ...
        for enemy in invaders:
            ...
            # enemy.hideturtle() # No need to hide sine splash screen alraedy over
        invaders_speed = 0.2

    # Game Completes Successfully
    if number_of_invaders > 3 * difficulty_factor: 
        ready = False
        wn.bgpic('ge2.gif')
        spaceship.hideturtle()
        missile.hideturtle()
    ...
    if ready:
        ...
        for enemy in invaders:  # NOTE: enemy is a single invader
            ...
            # Game Over OR invader-spaceship collision or the enemy cross the ship
            if enemy.distance(spaceship) < 25 or enemy.ycor() < -300:
                play_sound('exp.wav')
                enemy.hideturtle()
                spaceship.hideturtle()
                missile.hideturtle()
